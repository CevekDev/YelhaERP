...
interface Company {
  id: string; name: string; plan: string; email: string; wilaya: string | null
...
type Tab = 'stats' | 'companies' | 'users' | 'payments' | 'pricing'
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {stats.recentPayments.map((p, i) => (
                <tr key={p.id} className={`border-b border-zinc-800/50 hover:bg-zinc-800/30 ${i % 2 === 0 ? '' : ''}`}>
...
function PricingTab() {
  ...
  useEffect(() => {
    fetch('/api/admin/app-pricing').then(r => r.json()).then(d => {
      // apiSuccess wraps in data or returns directly
      const result = d.data ?? d
      setData(result)
      // Init edits with effective prices
