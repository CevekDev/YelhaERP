...
import { Check, CreditCard, Building2, Loader2, Copy, MessageCircle, ArrowLeft } from 'lucide-react'
...
export default function UpgradePage() {
  ...
  const whatsappText = plan
    ? encodeURIComponent(`Bonjour, je souhaite souscrire au plan ${plan.name} de YelhaERP (${plan.price.toLocaleString('fr-DZ')} DA/mois). Entreprise : ${session?.user?.companyName ?? ''}. Voici ma preuve de paiement :`)
    : ''
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">Changer de plan</h1>
          ...
          {PLANS.map(p => {
            ...
            return (
              ...
              >
                ...
                <p className="text-3xl font-bold">{p.price.toLocaleString('fr-DZ')} <span className="text-base font-normal text-muted-foreground">DA{p.period}</span></p>
      ...
      {selectedPlan && (
        <div className="space-y-3">
          <h2 className="font-semibold text-lg">2. Mode de paiement</h2>
      ...
      {selectedPlan && payMethod === 'chargily' && plan && (
        <div className="border rounded-2xl p-6 bg-card space-y-4">
          <h2 className="font-semibold text-lg">3. Paiement en ligne</h2>
          <div className="flex items-center justify-between bg-muted/50 rounded-xl p-4">
            <div>
              <p className="font-medium">Plan {plan.name}</p>
              <p className="text-sm text-muted-foreground">Abonnement mensuel</p>
            </div>
            <p className="text-xl font-bold">{plan.price.toLocaleString('fr-DZ')} DA</p>
          </div>
          ...
          <Button onClick={handleChargily} disabled={loading} className="w-full" size="lg">
            {loading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Redirection...</> : <><CreditCard className="w-4 h-4 mr-2" />Payer {plan.price.toLocaleString('fr-DZ')} DA en ligne</>}
          </Button>
        </div>
      )}
      ...
      {selectedPlan && payMethod === 'ccp' && plan && (
        <div className="border rounded-2xl p-6 bg-card space-y-4">
          <h2 className="font-semibold text-lg">3. Virement CCP</h2>
          <ol className="space-y-3 text-sm text-muted-foreground">
            <li className="flex gap-3"><span className="w-6 h-6 rounded-full bg-yelha-100 text-yelha-700 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">1</span>Effectuez un virement de <strong className="text-foreground">{plan.price.toLocaleString('fr-DZ')} DA</strong> vers le compte CCP ci-dessous</li>
            <li className="flex gap-3"><span className="w-6 h-6 rounded-full bg-yelha-100 text-yelha-700 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">2</span>Indiquez en référence : <span className="font-mono bg-muted px-1.5 py-0.5 rounded text-foreground">YELHA-{plan.id}-{session?.user?.companyName?.slice(0, 8).toUpperCase()}</span></li>
            <li className="flex gap-3"><span className="w-6 h-6 rounded-full bg-yelha-100 text-yelha-700 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">3</span>Envoyez la preuve de virement par WhatsApp — votre plan sera activé sous 24h ouvrables</li>
          </ol>
